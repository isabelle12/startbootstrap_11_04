    ...
    while($result_parfum_absent = $champs->fetch()){
        ...
        if($result_parfum_absent['parfum_absent']==1 AND $result_parfum_absent['contenance_absent']==1){ 
            ...
            while($result = $req_produit_cat->fetch()){
                ...
                $chaine_produit=$result['nom']." ".$result['quantite']." ".$result['nom_parfum'];
        ...
        if($result_parfum_absent['parfum_absent']==0 AND $result_parfum_absent['contenance_absent']==1){
            ...
            while($result = $req_volume->fetch()){
                ...
                $chaine_produit=$result['nom']." ".$result['quantite'];
                $tableau_qte[]=$result['article_qte'];
                $tableau_prix_vente[]=$result['prix_vente'];
...
$tableau_des_qtes = $_POST['tableau_inputs'];
for($i=0;$i<count($grand_tableau);$i++){
    /*var_dump($grand_tableau[$i]);
    var_dump($grand_tableau_qte[$i]);*/
    $insertion = $bdd->prepare("INSERT INTO commandes(numero_commande,date_commande,date_alerte,produit,qte,nom_distributeur) VALUES(:numero_commande,:date_commande,:date_alerte,:produit,:qte,:nom_distributeur)");
    $insertion->execute(array(
        ':numero_commande' => $chaine3,
        ':date_commande' => $date,
        ':date_alerte' => $date_convertie,
        ':produit' => $grand_tableau[$i],
        ':qte' => $tableau_des_qtes[$i],
...
for($i=0;$i<count($tableau_produits);$i++){
    ...
    foreach($grand_tableau as $cellule){
        $chaine4.=$cellule." ";
    }
