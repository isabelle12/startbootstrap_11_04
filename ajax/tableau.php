...
foreach($tableau as $cellule){
    ...
    while($result_parfum_absent = $champs->fetch()){
        ...
        if($result_parfum_absent['parfum_absent']==0 AND $result_parfum_absent['contenance_absent']==0){
            $req_volume = $bdd->prepare("SELECT prix_au_DV,articles_id.prix_vente AS prix_vente,articles_id.quantite AS article_qte,categories.nom,articles_id.id AS id_article2 FROM articles_id INNER JOIN categories ON articles_id.id_categorie = categories.id WHERE articles_id.id ='".$id."'");
            $req_volume->execute(array());
            //$req_volume = $req_volume->fetchAll();    
            $result = $req_volume->fetch();  
            $chaine = $result['nom'];
...
$data=array(
    ...
    'tableau2'=> $_POST["tableau_inputs"],
    'tableau_prix'=> $tableau_prix
